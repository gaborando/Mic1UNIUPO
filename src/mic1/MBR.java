...
/**
* mic1.Register that stores bytes fetched from memory.
*
* @author 
...
//    this.store_cl = store_cl;
  ...
  public void store() {
    if (memory_cl.getValue()[0]) {
      ...
      if (mic1sim.debug)
	DebugFrame.text.append("mic1.MBR: Store 0x" + Integer.toHexString(value).toUpperCase() + "\n");
      if (!mic1sim.run)
	setText("0x" + Integer.toHexString(value).toUpperCase());
    }
  }
  ...
  public void put() {
    if (mbr_cl.getValue()[0]) {
      if (mic1sim.debug)
	DebugFrame.text.append("mic1.MBR: Put 0x" + Integer.toHexString(value).toUpperCase() + "\n");
      if ((value & 128) == 0)
