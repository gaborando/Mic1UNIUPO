...
public class IJVMAssembler implements Mic1Constants
{
	...
	public IJVMAssembler(InputStream in, OutputStream out, String outfile, PrintStream err, String config)
	{
		...
		if (parse())
		{
			try
			{
				...
				generate();
				String debug = outfile.substring(0, index) + ".dbg";
				BufferedWriter out_dbg = new BufferedWriter(new FileWriter(debug));
				generatDebugMapping(out_dbg);
	...
	private void generatDebugMapping(BufferedWriter out_dbg) throws IOException
	{
		int startAddress = 0;
		for (IJVMInstruction instruction : main.getCode())
		{
			int instructionLine = (instruction.getAddress()+startAddress);
			int codeLine = instruction.getLineno();
			out_dbg.write(instructionLine+","+codeLine+"\n");
		}
		startAddress+=main.getByteCount();
		for(IJVMMethod method: methods){
			startAddress+=4;
			for(IJVMInstruction instruction: method.getCode()){
				int instructionLine = (instruction.getAddress()+startAddress);
				int codeLine = instruction.getLineno();
				out_dbg.write(instructionLine+","+codeLine+"\n");
			}
			startAddress+=method.getByteCount();
		}
		out_dbg.close();
	}
