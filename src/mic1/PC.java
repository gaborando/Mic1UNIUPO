...
public class PC extends TextField {
  ...
  public void store() {
    if (store_cl.getValue()[0]) {
      value = in.getValue();
      if (mic1sim.debug) 
	DebugFrame.text.append("mic1.PC: Store " + Integer.toHexString(value).toUpperCase() + "\n");
  ...
  public void put() {
    if (put_cl.getValue()[0]) {
      out.setValue(value);
      if (mic1sim.debug) 
	DebugFrame.text.append("mic1.PC: Put " + Integer.toHexString(value).toUpperCase() + "\n");
      if (!mic1sim.run || cycle_count == COUNT_MAX) {
	setText("0x" + Integer.toHexString(value).toUpperCase());
        cycle_count = 0;
      }
  ...
  public void mem() {
    if (mem_cl.getValue()[MIR.FETCH]) {
      ...
      if (mic1sim.debug) 
	DebugFrame.text.append("mic1.PC: Fetch byte " + Integer.toHexString(value).toUpperCase() + "\n");
      if (!mic1sim.run || cycle_count == COUNT_MAX) {
	setText("0x" + Integer.toHexString(value).toUpperCase());
        cycle_count = 0;
      }
